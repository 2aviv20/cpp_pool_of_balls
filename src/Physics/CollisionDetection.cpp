...
bool CollisionDetection::IsCollidingCircleCircle(Body* a, Body* b, Contact& contact) {
	CircleShape* A_circleShape = (CircleShape*)a->shape;
	CircleShape* B_circleShape = (CircleShape*)b->shape;

	Vec2 ab = b->position - a->position;
	float sumRadius = A_circleShape->radius + B_circleShape->radius;
	...
	contact.start = b->position - contact.normal * B_circleShape->radius;
	contact.end = a->position + contact.normal * A_circleShape->radius;
